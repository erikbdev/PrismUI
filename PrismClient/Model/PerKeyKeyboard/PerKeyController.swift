...
class PerKeyController: Controller {
    ...
    /// Uploads data from an array of objects. In our case, you send keys and effects to update.
    ///
    ...
    func update(data: Any, force: Bool) {
        ...
        commandMutex.async { [unowned self] in
            ...
            let effects = updateKeys.compactMap({ $0.effect }).uniqued()

            var effectsWithId = [UInt8: Key.Effect]()

            // Now we generate an id for the effect.
            for (index, element) in effects.enumerated() {
                effectsWithId[UInt8(index + 1)] = element
            }
            ...
            if updateModifiers {
                lastByte = 0x2d
                let result = self.writeKeysToKeyboard(
                    keys: updateKeys,
    ...
    private func writeKeysToKeyboard(keys: [Key], region: UInt8, keycodes: [UInt8], effectsWithId: [UInt8: Key.Effect]) -> IOReturn {
        var data = Data(capacity: PerKeyProperties.packageSize)
            ...
            if let key = keyboardKeys.filter({ $0.region == region && $0.keycode == keyCode }).first {
                ...
                data.append(
                    [
                        ...
                        UInt8(key.effect.duration & 0x00ff),
                        UInt8(key.effect.duration >> 8),
                        effectId,
                        mode
                    ],
                    count: 10
