...
struct PerKeyKeyboardState: Equatable {
    var keys: IdentifiedArrayOf<KeyCore.State> = []
    var selected = Set<Int>()
    var model: Models = .unknown
}

enum PerKeyKeyboardAction: Equatable {
    case onAppear
    case loadKeys
...
struct PerKeyKeyboardEnvironment {}

let perKeyKeyboardReducer: Reducer<PerKeyKeyboardState, PerKeyKeyboardAction, PerKeyKeyboardEnvironment> = .combine(
    KeyCore.reducer.forEach(
        state: \.keys,
        action: /PerKeyKeyboardAction.key(id:action:),
